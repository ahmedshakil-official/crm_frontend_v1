        ...
        <Col xs="12">
          <div>
            ...
            <div className="avatar-preview">
              <div id="image" style={{backgroundImage: imageUrl ? `url(${imageUrl})` : `url(${`${ImagePath}/avatars/1.jpg`}`,}} />
            </div>
          </div>
          <h6>{AddProfile}</h6>
        </Col>
        <Col xs="12">
          <Label check className="form-label">{PortfolioURL}</Label>
          <Input value={portfolioURL} onChange={getUserData} name="portfolioURL" type="url" placeholder="https://dunzo" />
        </Col>
        <Col xs="12">
          <Label check className="form-label">{ProjectDescription}</Label>

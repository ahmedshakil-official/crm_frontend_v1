  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      if (user) {
        ...
        // console.log("Access Token in JWT Callback:", userWithToken.token);
    ...
    async session({ session, token }) {
      ...
      session.user = {
        ...session.user,
        accessToken: token.accessToken as string | undefined, // Ensure compatibility
      };

      // Log the session object for debugging
      // console.log("Access Token in Session:", session.user.accessToken);
